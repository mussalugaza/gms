...
public function prefs_init($pref, $value, $username, $scope_ob)
    {
      ...
      switch ($pref) {
        case 'from_addr':
            ...
            $searchBase = 'ou=Users,domainName="'.$vdomain.'",o=domains,dc=gov,dc=go,dc=tz';
            $cmd = '/usr/bin/ldapsearch -ZZ -x -b ' . $searchBase . ' uid=' . escapeshellcmd($username) . ' mail | /bin/grep mail: | /usr/bin/awk \'{print $2}\'';
...
public function preauthenticate($userId, $credentials)
{
 ...
 // $vdomain ='ega.go.tz'; 
  ...
  $result['userId'] = strtolower($userId); 
