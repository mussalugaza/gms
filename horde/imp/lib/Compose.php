...
class IMP_Compose implements ArrayAccess, Countable, IteratorAggregate
{
    ...
    )
    {
        ...
        ###########################################################################################################
        $sigNature =$identity;
        if((intval($this->replyType(true)))==self::REPLY or (intval($this->replyType(true)))==self::REPLY ){
            $sigNature ='';
         }
         else {
...
/*        $msg_options = array(
    ...
    public function replyMessageText($contents, array $opts = array())
    {
        ...
        //add signature to the message
            ...
            $signature=$identity->getValue('signature_html');
            //syslog (LOG_NOTICE, 'kibwiru'.($signature) );
            //die();
            //$signature ='';
        return array(
            'body' => "\n \n<br>---------- \n <br>".$signature.$msg . "\n",
...
##############################################################################
        ...
        $x = $GLOBALS['injector']->getInstance('Horde_Core_Factory_TextFilter')->filter(
            $signature,
            'Xss',
            array(
                'charset' => 'UTF-8',
        ...
        $xp = new DOMXPath($x->dom);
        foreach ($xp->query('//*[@src]') as $y) {
            $o = $y->getAttribute('src');
            if(Horde_Url_Data::isData($o)){
                break;
            }
        }
