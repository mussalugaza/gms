...
class IMP_Dynamic_Mailbox extends IMP_Dynamic_Base
{
    ...
    protected function _init()
    {
        ...
        if ($imp_imap->isImap()) {
            ...
            $impSidebar->containers[] = array(
                'rows' => array(
                    array(
                        'id' => 'browse-contacts',
                        'cssClass' => 'horde-subnavi-icon turba-browse',
                        'link' => "<a href='/turba/browse.php?source=ldapgroup'>" . _("Contacts Groups") . '</a>'
                    )
                )
            );
        ...
        $sidebar->content = $impSidebar->render('sidebar').$impSidebar->render('cal_events');
